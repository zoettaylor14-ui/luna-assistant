...
function OracleSection() {
  ...
  return (
    <div className="space-y-5">
      ...
      {error && (
        <p className="text-center text-sm" style={{ color: 'var(--blush)' }}>{error}</p>
      )}
  ...
  async function analyzeImage(url: string) {
    ...
    try {
      const res = await fetch('/api/atelier/inspiration', {
        ...
        body: JSON.stringify({
          image_url: url,
          source: 'luna_reference',
          analyze: true,
          description: `LUNA fashion reference image from the style board`,
          is_preloaded: true,
        })
      })
      const data = await res.json()
      ...
      <div className="grid grid-cols-3 gap-2">
        {REFERENCE_IMAGES.map((src, i) => (
          <button key={i}
            ...
            style={{
              border: activeImage === src ? '2.5px solid var(--violet)' : '1px solid var(--surface-border)',
            }}>
            <img
              src={src}
              alt={`Style ref ${i + 1}`}
  ...
  async function seedProjects() {
    ...
    try {
      const res = await fetch('/api/atelier/projects', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ seed_starters: true }),
      })
      const data = await res.json()
  ...
  async function advanceStage(project: SewingProject) {
    ...
    try {
      const res = await fetch('/api/atelier/projects', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: project.id, status: nextStage }),
      })
      const data = await res.json()
      if (data.project) {
      ...
      {loading ? (
        ...
        <ACard className="text-center py-8">
          ...
          <button
            ...
            disabled={seeding}
            className="rounded-2xl px-6 py-3 font-bold text-sm transition-all"
            style={{ background: 'linear-gradient(135deg, #8B6FB8, #6A4F9B)', color: '#fff', opacity: seeding ? 0.7 : 1 }}>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <div className="flex-1 h-1.5 rounded-full" style={{ background: 'var(--surface-subtle)' }}>
                          <div className="h-1.5 rounded-full transition-all"
                            style={{ width: `${progress}%`, background: stageColor(project.status) }} />
                        </div>
                        <span className="text-xs font-medium flex-shrink-0"
                          style={{ color: stageColor(project.status) }}>
                          {project.status}
                        </span>
                      </div>
                    </div>
                    <ChevronDown className="w-4 h-4 flex-shrink-0 mt-1 transition-transform"
                      style={{ color: 'var(--text-3)', transform: isExpanded ? 'rotate(180deg)' : undefined }} />
                  </div>
                </button>
                  ...
                  <div className="mt-4 pt-4" style={{ borderTop: '1px solid var(--surface-border)' }}>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.materials_needed.map(m => (
                            <span key={m} className="text-sm px-3 py-1 rounded-full"
                              style={{ background: 'var(--surface)', border: '1px solid var(--surface-border)', color: 'var(--text-2)' }}>
                              {m}
                            </span>
                          ))}
                      ...
                      <div className="flex flex-wrap gap-1">
                        {PROJECT_STAGES.map((stage, idx) => (
                          <span key={stage}
                            ...
                            style={{
                              background: idx <= stageIdx ? 'rgba(139,111,184,0.2)' : 'var(--surface-subtle)',
                              color: idx <= stageIdx ? 'var(--violet)' : 'var(--text-4)',
                              border: idx === stageIdx ? '1px solid rgba(139,111,184,0.4)' : '1px solid transparent',
                            }}>
                            {idx <= stageIdx ? '✓ ' : ''}{stage}
                          </span>
                        ))}
                    ...
                    {!isDone && (
                      <button
                        ...
                        className="w-full rounded-2xl py-3 font-semibold text-sm transition-all flex items-center justify-center gap-2"
    ...
    <div className="space-y-5">
      ...
      <ACard>
        ...
        <input
          ...
          placeholder="e.g. Lace-insert cargos with crystal waist chain"
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {PROJECT_TYPES.map(t => (
            <button key={t.value}
              ...
              style={{
                background: projectType === t.value ? 'var(--violet)' : 'var(--surface)',
                color: projectType === t.value ? '#fff' : 'var(--text-2)',
                border: projectType === t.value ? 'none' : '1px solid var(--surface-border)',
              }}>
              {t.label}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {STYLE_LANES.map(lane => (
            <button key={lane}
              onClick={() => setStyleLane(lane)}
              className="rounded-full px-3 py-1.5 text-sm font-medium transition-all"
              style={{
                background: styleLane === lane ? 'rgba(139,111,184,0.20)' : 'var(--surface-subtle)',
                color: styleLane === lane ? 'var(--violet)' : 'var(--text-3)',
                border: styleLane === lane ? '1.5px solid rgba(139,111,184,0.4)' : '1px solid var(--surface-border)',
              }}>
              {lane}
            </button>
          ))}
        ...
        <textarea
          ...
          placeholder="Describe the vision — fabrics, fit, details, inspiration..."
          rows={4}
        ...
        <button
          ...
          disabled={saving || !projectTitle || !styleLane}
          className="w-full rounded-2xl py-3.5 font-bold text-sm transition-all flex items-center justify-center gap-2"
          style={{
            background: !projectTitle || !styleLane ? 'var(--surface)' : 'linear-gradient(135deg, #8B6FB8, #6A4F9B)',
            color: !projectTitle || !styleLane ? 'var(--text-3)' : '#fff',
            opacity: saving ? 0.7 : 1,
          }}>
          {saved ? <><Check className="w-4 h-4" /> Saved to Projects</> :
           saving ? 'Saving...' : <><Scissors className="w-4 h-4" /> Create Project</>}
        </button>
      </ACard>

      {/* Sewing machine setup guide */}
      <ACard>
        <SLabel>Sewing machine setup checklist</SLabel>
        <div className="space-y-2">
          {[
            'Thread the machine (top + bobbin)',
            'Set stitch type: straight stitch for seams, zigzag for stretch',
            'Test stitch on a scrap of your fabric first',
            'Use the right needle: ballpoint for jersey, sharp for woven',
            'Lower the presser foot before sewing',
            'Backstitch at start and end of every seam',
            'Press seams with iron after each step',
          ].map((step, i) => (
            <div key={i} className="flex items-start gap-3 rounded-xl p-3"
              style={{ background: 'var(--surface-subtle)', border: '1px solid var(--surface-border)' }}>
              <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                style={{ background: 'rgba(139,111,184,0.15)', border: '1px solid rgba(139,111,184,0.2)' }}>
                <span className="font-bold" style={{ color: 'var(--violet)', fontSize: '0.65rem' }}>{i + 1}</span>
              </div>
  ...
  async function seedVault() {
    ...
    try {
      const res = await fetch('/api/atelier/vault', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ seed_vault: true }),
      })
      const data = await res.json()
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {concepts.map(concept => {
            ...
            return (
              <div key={concept.id} className="rounded-3xl overflow-hidden"
                style={{
                  background: categoryGradient(concept.category),
                  border: '1px solid var(--surface-border)',
                }}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs" style={{ color: 'var(--text-3)' }}>
                          {concept.status === 'vault' ? '🔒 In Vault' : '🚀 Active'}
                        </span>
                      </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-2">
                            {concept.product_ideas.map((prod, idx) => (
                              <div key={idx} className="rounded-2xl p-3"
                                style={{ background: 'var(--surface)', border: '1px solid var(--surface-border)' }}>
          ...
          {/* Add new concept CTA */}
          <div className="rounded-3xl p-5 flex flex-col items-center justify-center gap-2 text-center"
            style={{ background: 'var(--surface)', border: '1.5px dashed var(--surface-border)' }}>
...
export default function AtelierPage() {
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case 'closet':      return <ClosetSection />
      case 'inspiration': return <InspirationSection />
