...
export async function POST(req: NextRequest) {
  try {
    ...
    const account = await stripe.financialConnections.accounts.retrieve(account_id)
    ...
    const txnList = await (stripe.financialConnections.accounts as any).listTransactions(account_id, { limit: 200 }) as { data: any[] }
